...
class BlackScholesCalculator:
    ...
    def get_nifty_expiry_dates(self) -> List[datetime]:
        """Get next 3 monthly expiry dates for Nifty options."""
        today = datetime.now()
        expiry_dates = []
        ...
        for month_offset in range(3):  # Next 3 months
            ...
            if days_back == 0 and last_day.day < 7:  # If it's Thursday but first week
                days_back = 7
            
            expiry = last_day - timedelta(days=days_back)
            
            # If expiry is in the past or today, skip to next month
            if expiry <= today:
                continue
                
            expiry_dates.append(expiry)
        
        return expiry_dates[:3]  # Return max 3 expiry dates
