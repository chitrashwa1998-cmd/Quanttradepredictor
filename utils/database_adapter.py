"""Error handling added for potential KeyError exceptions during model loading."""
"""
...
class DatabaseAdapter:
    ...
    def load_model_results(self, model_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            results = self.db.load_model_results(model_name)
            return results if results is not None else {}
        except KeyError as e:
            print(f"KeyError loading model results for {model_name}: {str(e)}")
            return {}
        except Exception as e:
            print(f"Error loading model results for {model_name}: {str(e)}")
            return {}
    ...
    def load_trained_models(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            models = self.db.load_trained_models()
            return models if models is not None else {}
        except KeyError as e:
            print(f"KeyError loading trained models: {str(e)}")
            return {}
        except Exception as e:
