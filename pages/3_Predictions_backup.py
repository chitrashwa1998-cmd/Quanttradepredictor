...
try:
    ...
    # Create a safe index for the prediction dataframe
    safe_index = features_filtered.index
        ...
        with tab4:
            ...
            if len(display_df.columns) > 0:
                ...
                try:
                    ...
                    date_series = pd.to_datetime(display_df[index_col_name])
                    display_df['Date'] = date_series.dt.strftime('%Y-%m-%d %H:%M')
                except:
                    # If conversion fails, use string representation
                    display_df['Date'] = display_df[index_col_name].astype(str)
                
                # Remove the original index column if it exists and is different from Date
                if index_col_name != 'Date' and index_col_name in display_df.columns:
            ...
            try:
                sorted_df = display_df.tail(50).sort_values('Date', ascending=False) if 'Date' in display_df.columns else display_df.tail(50)
            except:
                sorted_df = display_df.tail(50)
